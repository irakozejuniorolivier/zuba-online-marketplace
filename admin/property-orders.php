...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['change_status'])) {
        ...
        $allowed = ['payment_submitted', 'approved', 'processing', 'completed', 'cancelled', 'rejected'];
        if (in_array($new_status, $allowed)) {
            $conn->query("UPDATE property_orders SET status = '$new_status' WHERE id = $order_id");
            if ($new_status === 'approved') {
                $conn->query("UPDATE property_orders SET approved_by = {$admin['id']}, approved_at = NOW() WHERE id = $order_id");
            }
            logActivity('admin', $admin['id'], 'CHANGE_PROPERTY_ORDER_STATUS', "Changed property order #$order_id status to $new_status");
            setFlash('success', 'Property order status updated successfully.');
        }
        redirect('property-orders.php');
    }
    
    if (isset($_POST['delete_id'])) {
        $del_id = (int)$_POST['delete_id'];
        $conn->query("DELETE FROM property_orders WHERE id = $del_id");
        logActivity('admin', $admin['id'], 'DELETE_PROPERTY_ORDER', "Deleted property order ID $del_id");
        setFlash('success', 'Property order deleted successfully.');
        redirect('property-orders.php');
    }
}
...
if ($search !== '') {
    $where[] = "(po.order_number LIKE ? OR u.name LIKE ? OR u.email LIKE ? OR po.property_title LIKE ?)";
    $s = "%$search%";
...
// Total count
$count_sql = "SELECT COUNT(*) FROM property_orders po LEFT JOIN users u ON po.user_id = u.id WHERE $where_sql";
if ($params) {
...
// Fetch orders
$sql = "SELECT po.*, u.name AS customer_name, u.email AS customer_email, pm.name AS payment_method_name
...
// Stats
$stats_bar = [];
foreach (['payment_submitted', 'approved', 'processing', 'completed', 'cancelled', 'rejected'] as $s) {
    $r = $conn->query("SELECT COUNT(*) FROM property_orders WHERE status='$s'");
    $stats_bar[$s] = (int)$r->fetch_row()[0];
}
$stats_bar['total'] = (int)$conn->query("SELECT COUNT(*) FROM property_orders")->fetch_row()[0];
...
.property-title { font-weight:600; color:var(--text); }
.order-type-badge { display:inline-block; padding:4px 8px; border-radius:4px; font-size:11px; font-weight:600; }
.order-type-purchase { background:#dbeafe; color:#1e40af; }
.order-type-rent { background:#d1fae5; color:#065f46; }
...
<!-- Filters -->
<form method="GET" action="property-orders.php">
<div class="filters-bar">
    <div class="filter-group">
        <label>Search</label>
        <input type="text" name="search" placeholder="Order #, customer, property…" value="<?= e($search) ?>">
    </div>
    <div class="filter-group">
        <label>Status</label>
        <select name="status">
    ...
    <div class="filter-group">
        ...
        <select name="order_type">
            ...
            <option value="rent" <?= $order_type==='rent' ? 'selected':'' ?>>Rent</option>
        </select>
    </div>
    <div class="filter-actions">
        <button type="submit" class="btn btn-primary">Filter</button>
        <a href="property-orders.php" class="btn" style="background:#f3f4f6;color:var(--text);">Reset</a>
    </div>
</div>
</form>
...
<div class="card">
    ...
    <div class="empty-state">
        ...
        <h4>No property orders found</h4>
        <p><?= $search || $status || $order_type ? 'Try adjusting your filters.' : 'No property orders yet.' ?></p>
    </div>
    <?php else: ?>
        ...
        <table class="orders-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="property-title"><?= e($o['property_title']) ?></span></td>
                    <td>
                        <span class="order-type-badge order-type-<?= $o['order_type'] ?>">
                    ...
                    <td>
                        <div class="action-btns">
                            <a href="property-order-details.php?id=<?= $o['id'] ?>" class="action-btn ab-view" title="View Details">
    ...
    <div class="mob-card">
        ...
        <div class="mob-order">
            <div class="mob-order-header">
                ...
                <span class="order-type-badge order-type-<?= $o['order_type'] ?>"><?= ucfirst($o['order_type']) ?></span>
            </div>
            <div class="mob-order-body">
                ...
                <div class="mob-row">
                    <span class="mob-row-label">Property:</span>
                    <span class="mob-row-value"><?= e($o['property_title']) ?></span>
                </div>
                <div class="mob-row">
                    <span class="mob-row-label">Amount:</span>
                    <span class="mob-row-value"><?= formatCurrency($o['amount']) ?></span>
                </div>
                <div class="mob-row">
            ...
            <div class="mob-order-footer">
                ...
                <a href="property-order-details.php?id=<?= $o['id'] ?>" class="action-btn ab-view">
                    <svg viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                </a>
    ...
    <div class="pagination">
        ...
        <div class="pagination-links">
            <?php
            $q = http_build_query(array_filter(['search'=>$search,'status'=>$status,'order_type'=>$order_type]));
            $base = 'property-orders.php?' . ($q ? $q.'&' : '');
            ?>
...
<div class="modal" id="deleteModal">
    <div class="modal-content">
        <div class="modal-header">Delete Property Order</div>
