...
$id = (int)($_GET['id'] ?? 0);
if (!$id) redirect('property-orders.php');

// Fetch order with customer, property, and payment method info
$order = $conn->query("SELECT po.*, u.name AS customer_name, u.email AS customer_email, u.phone AS customer_phone,
                       pm.name AS payment_method_name, pm.account_name, pm.account_number,
                       a.name AS approved_by_name,
                       p.slug AS property_slug,
                       (SELECT image_path FROM property_images WHERE property_id = po.property_id AND is_primary = 1 LIMIT 1) AS property_image
...
if (!$order) redirect('property-orders.php');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update_status'])) {
        ...
        if (in_array($new_status, $allowed)) {
            $stmt = $conn->prepare("UPDATE property_orders SET status = ?, admin_note = ? WHERE id = ?");
            $stmt->bind_param("ssi", $new_status, $admin_note, $id);
            $stmt->execute();
            $stmt->close();
            
            if ($new_status === 'approved' && $order['status'] !== 'approved') {
                $conn->query("UPDATE property_orders SET approved_by = {$admin['id']}, approved_at = NOW() WHERE id = $id");
            }
            
            logActivity('admin', $admin['id'], 'UPDATE_PROPERTY_ORDER', "Updated property order #{$order['order_number']} status to $new_status");
            setFlash('success', 'Property order updated successfully.');
            redirect("property-order-details.php?id=$id");
        }
    }
}
...
.property-preview { display:flex; gap:16px; align-items:start; }
.property-thumb { width:120px; height:120px; border-radius:8px; object-fit:cover; border:1px solid var(--border); background:#f3f4f6; }
.property-thumb-placeholder { width:120px; height:120px; border-radius:8px; background:#f3f4f6; border:1px solid var(--border); display:flex; align-items:center; justify-content:center; }
.property-info { flex:1; }
.property-info h4 { margin:0 0 8px; font-size:16px; color:var(--text); }
.property-info p { margin:4px 0; font-size:13px; color:var(--text-muted); }
.order-type-badge { display:inline-block; padding:4px 10px; border-radius:4px; font-size:11px; font-weight:700; text-transform:uppercase; }
...
@media (max-width:640px) {
    ...
    .property-preview { flex-direction:column; }
    .property-thumb, .property-thumb-placeholder { width:100%; height:200px; }
}
</style>

<div class="page-header">
    <h2>🏠 Property Order #<?= e($order['order_number']) ?></h2>
    <a href="property-orders.php" class="btn-back">← Back to Orders</a>
</div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="property-preview">
                    <?php if ($order['property_image']): ?>
                        <img src="<?= UPLOAD_URL . 'properties/' . e($order['property_image']) ?>" class="property-thumb" alt="">
                    <?php else: ?>
                        <div class="property-thumb-placeholder">
                            <svg viewBox="0 0 24 24" style="width:48px;height:48px;stroke:#d1d5db;fill:none;stroke-width:1.5;">
                    ...
                    <div class="property-info">
                        <h4><?= e($order['property_title']) ?></h4>
                        <p><strong>Order Type:</strong> <span class="order-type-badge order-type-<?= $order['order_type'] ?>"><?= ucfirst($order['order_type']) ?></span></p>
                        <?php if ($order['order_type'] === 'rent' && $order['rent_duration']): ?>
                        <p><strong>Rent Duration:</strong> <?= $order['rent_duration'] ?> <?= e($order['rent_period']) ?>(s)</p>
                        <?php endif; ?>
                        <p><strong>Amount:</strong> <span style="font-size:18px;font-weight:700;color:var(--orange);"><?= formatCurrency($order['amount']) ?></span></p>
                    </div>
                </div>
            </div>
        </div>
