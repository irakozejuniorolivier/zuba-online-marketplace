...
// Order status steps
$status_steps = [
    'pending_payment' => ['label' => 'Pending Payment', 'icon' => 'fa-clock', 'color' => '#f59e0b'],
    ...
    'processing' => ['label' => 'Processing', 'icon' => 'fa-box', 'color' => '#8b5cf6'],
    'shipped' => ['label' => 'Shipped', 'icon' => 'fa-truck', 'color' => '#06b6d4'],
    'delivered' => ['label' => 'Delivered', 'icon' => 'fa-check-double', 'color' => '#059669']
];
...
<head>
    ...
    <title>Order #<?= e($order_number) ?> | Zuba Online Market</title>
...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .container { max-width: 900px; margin: 0 auto; padding: 24px 20px; }
        ...
        .order-item { display: flex; gap: 12px; padding: 12px; background: #f9fafb; border-radius: 10px; margin-bottom: 10px; border: 1px solid #e5e7eb; }
        .item-img { width: 70px; height: 70px; border-radius: 8px; overflow: hidden; background: #fff; flex-shrink: 0; border: 1px solid #e5e7eb; }
        .item-img img { width: 100%; height: 100%; object-fit: cover; }
        .item-img .no-img { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; color: #d1d5db; font-size: 28px; }
        .item-info { flex: 1; min-width: 0; display: flex; flex-direction: column; justify-content: center; }
        .item-name { font-size: 14px; font-weight: 700; color: #1a1a2e; margin: 0 0 6px; line-height: 1.3; }
        .item-details { font-size: 12px; color: #6b7280; margin: 0 0 6px; }
        .item-price { font-size: 15px; font-weight: 800; color: #f97316; }
        ...
        @media (max-width: 640px) {
            ...
            .order-item { gap: 10px; padding: 10px; }
            .item-img { width: 65px; height: 65px; }
            .item-img .no-img { font-size: 24px; }
            .item-name { font-size: 13px; margin-bottom: 4px; }
            .item-details { font-size: 11px; margin-bottom: 4px; }
            ...
            <div class="card-header">
                ...
                <div class="tracking-container">
                    ...
                    <?php else: ?>
                        <!-- Normal flow -->
                        <?php
                        $steps_order = ['pending_payment', 'payment_submitted', 'approved', 'processing', 'shipped', 'delivered'];
                                ...
                                <div class="step-content">
                                    ...
                                    <p class="step-desc">
                                        ...
                                        $descriptions = [
                                            ...
                                            'approved' => 'Payment verified and order approved.',
                ...
                <div class="order-item">
                    <div class="item-img">
                        <?php if ($item['image_path']): ?>
                            <img src="<?= UPLOAD_URL . 'products/' . $item['image_path'] ?>" alt="<?= e($item['product_name']) ?>">
                        <?php else: ?>
                            <div class="no-img"><i class="fas fa-box-open"></i></div>
                        <?php endif; ?>
                    </div>
                    <div class="item-info">
                        <h3 class="item-name"><?= e($item['product_name']) ?></h3>
                        <p class="item-details">
                            Qty: <?= $item['quantity'] ?> × <?= formatCurrency($item['price']) ?>
                        </p>
                        <p class="item-price"><?= formatCurrency($item['subtotal']) ?></p>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
        ...
        <div class="card">
            ...
            <?php if ($order['shipping_fee'] > 0): ?>
                <div class="summary-row">
                    ...
                    <span class="value"><?= formatCurrency($order['shipping_fee']) ?></span>
                </div>
            <?php endif; ?>
        ...
        <div class="card">
            ...
            <div class="info-grid">
                <div class="info-item">
                    ...
                    <div class="info-value"><?= e($order['shipping_name']) ?></div>
                </div>
                <div class="info-item">
                    <div class="info-label">Phone Number</div>
                    <div class="info-value"><?= e($order['shipping_phone']) ?></div>
                </div>
                <div class="info-item" style="grid-column: 1 / -1;">
                    <div class="info-label">Address</div>
                    <div class="info-value"><?= e($order['shipping_address']) ?></div>
                </div>
                <div class="info-item">
                    <div class="info-label">City</div>
                    <div class="info-value"><?= e($order['shipping_city']) ?></div>
                </div>
                <div class="info-item">
                    <div class="info-label">Country</div>
                    <div class="info-value"><?= e($order['shipping_country']) ?></div>
                </div>
            </div>
        ...
        <div class="card">
            ...
            <?php if ($order['payment_proof']): ?>
                <div class="payment-proof-container">
                    ...
                    <img src="<?= UPLOAD_URL . 'payment_proofs/' . e($order['payment_proof']) ?>" alt="Payment Proof" class="payment-proof" onclick="openProofModal(this.src)">
                </div>
            <?php endif; ?>
...
<!-- Payment Proof Modal -->
<div id="proofModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.85); z-index:9999; align-items:center; justify-content:center; padding:20px;" onclick="closeProofModal()">
    <button onclick="closeProofModal()" style="position:absolute; top:16px; right:16px; width:40px; height:40px; border-radius:50%; background:#fff; border:none; font-size:22px; cursor:pointer; display:flex; align-items:center; justify-content:center; line-height:1;">×</button>
    <img id="proofModalImg" src="" alt="Payment Proof" style="max-width:90%; max-height:90vh; border-radius:8px; box-shadow:0 20px 40px rgba(0,0,0,0.4);" onclick="event.stopPropagation()">
</div>
